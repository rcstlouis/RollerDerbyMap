...
const filteredLeagues = computed<LeagueRecord[]>(() => {
  const fl = (wftdaScrape as unknown as LeagueRecord[]).sort((a, b) => {
  ...
  return fl.filter(
    (league) =>
      league.city.toLocaleLowerCase().includes(filter.value.toLocaleLowerCase()) ||
      league.name.toLocaleLowerCase().includes(filter.value.toLocaleLowerCase()),
  )
})
</script>
